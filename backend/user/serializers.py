...
class AddFriendSerializer(serializers.ModelSerializer):
    ...
    def sentRequest(self, validated_data):
        sender = self.context['sender'].user
        recipient = validated_data['recipient']

        if sender == recipient:
            raise serializers.ValidationError('you cant add yourself as a friend')
        
        if FriendRequest.objects.filter(sender=sender, recipient=recipient).exists():
            raise serializers.ValidationError('send request exist')
